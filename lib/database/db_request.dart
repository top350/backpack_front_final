...
// List<RequestObject> othersList = sortCategory('Others',requestList);
  ...
  List<RequestObject> sortCategory(String category, List<RequestObject> item) {
    //sort Category
    List<RequestObject> sortedList = [];
    for (int i = 0; i < item.length; i++) {
      if (item[i].itemCategory == category) sortedList.add(item[i]);
    }
    return sortedList;
  }
...
class RequestList {
  ...
  factory RequestList.fromJson(List<dynamic> parsedJson) {
    ...
    return new RequestList(request_list: request_list);
  }
}
